...
class AmazonScraper {
    ...
    get setRequestEndpoint() {
        switch (this.scrapeType) {
            case 'products':
                return 's';
            case 'reviews':
                return `product-reviews/${this.asin}/ref=cm_cr_arp_d_viewopt_srt${this.reviewFilter.pageNumber ? `?pageNumber=${this.reviewFilter.pageNumber}` : ''}&formatType=${
    ...
    grapAsinDetails(body) {
        ...
        let coupon_text_raw = $(coupon_div).text()
        let coupon_text = (coupon_text_raw.split('.')[0]).replace('Coupon:','').trim()
        let coupon_terms = (coupon_text_raw.split('.')[1]).replace('Terms','').trim()
