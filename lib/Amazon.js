...
const fs = require('fs');
const { uploadFileHTML, htmlKey,readFileHTML } = require("./lib");
...
class AmazonScraper {
    ...
    async buildRequest(page) {
        ...
        try {
            ...
            let yesterday = moment().subtract(1, 'days').utc().format('yyyy-MM-DD')

            let html_key_today = await htmlKey(this.s3, this.s3_bucket_name, `variations/${today}/`, instance_asin)
            let html_key_yesterday = await htmlKey(this.s3, this.s3_bucket_name, `variations/${yesterday}/`, instance_asin)
            
            if(html_key_today.length || html_key_yesterday.length){
                // Page scraped recently
                let html_key = html_key_today.length ? html_key_today : html_key_yesterday;
                let html_file = await readFileHTML(this.s3, this.s3_bucket_name, html_key[0].Key);
                console.log('# # # HTML Already Exists');
                return html_file
            }else{
                // There is no recent scrape
                console.log('# # # New HTML Scrapping');
                const response = await this.httpRequest(options);
                uploadFileHTML(this.s3, this.s3_bucket_name, this.s3_parent_folder, `${instance_asin}-${moment().utc().format('yyyy-MM-DD-HH-mm-ss')}.html`, response.body)
    ...
    extractRelatedProducts($) {
        ...
        try {
            for (let i = 0; i < rp_cards.length; i++) {
                ...
                let rp_url = `https://www.amazon.com/dp/${rp_asin}`
